...
pub fn detect_agent_source(
    ...
    if is_opencode_command(command, window_name) {
...
fn is_opencode_command(command: &str, window_name: Option<&str>) -> bool {
    command == "node" && window_name.is_some_and(|name| name.to_lowercase().contains("opencode"))
}
...
mod tests {
    ...
    fn test_detect_agent_source_claude_and_opencode() {
        ...
        assert_eq!(detect_agent_source("node", Some("project"), None), None);
    }
}
